"""
Standard CNN trained on upright MNIST only (baseline).

This baseline model establishes a performance floor - trained on upright (0°)
MNIST data only. Expected behavior: poor robustness to rotations, good accuracy
on upright test set.
"""
...
def train_standard_cnn_baseline(
    data_dir: str,
    output_dir: str,
    num_epochs: int = 10,
    batch_size: int = 32,
    ...
    """
    Train Standard CNN on upright MNIST only.
    ...
    # Save model
    output_path = Path(output_dir) / "standard_cnn_baseline.pt"
